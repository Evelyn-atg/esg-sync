...
崩溃捕获（`src/chandra_ocr.py`，2026-08-02 前为 `chandra_ocr_tester.py`）：捕获 `RuntimeError` 且含 `CUDA` 时，标记坏卡 → 该 PDF 入黑名单 → `raise GPUFatalError`。`numeric_extractor.py` 收到 `GPUFatalError` 后 `sampler.stop()` + 立即退出。
