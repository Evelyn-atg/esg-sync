...
1. **工作负载均衡**：基于 OCR 缓存动态分配任务
2. **特殊 PDF 处理**：检测并跳过异常 PDF
...
#### 为什么 H100 当前无法使用（2026-07-31）

综合上述现象，集群中我们能实际调度的 H100 处于「不可用」状态。根因不是配置或代码，而是**坏卡 + 调度无健康感知 + 单卡限制**三重叠加：

1. **坏卡是唯一能摸到的 H100**：`ssh gpu3` 实测仅枚举到 1 张 H100，即坏掉的 GPU 0（Bus `00000000:23:00.0`）。文档中「GPU 1 正常运行（Job 1014）」对应的另一张 H100 位于其他计算节点，本 OCR 任务用不到。
2. **Slurm 看不见故障**：`scontrol show node gpu3` 仍显示节点 IDLE、2 卡可用（其中 GPU 0 实际已坏），调度器持续把新 H100 任务分到坏卡。于是 Job 1025 一上去就踩中 GPU 0，跑约 5 小时后 `CUDA error: unspecified launch failure` → `GPU requires reset`，三个 stream 日志全 0 条 OCR SUMMARY，零产出。
3. **无法自助隔离到健康卡**：
   - `pam_slurm_adopt` 限制：没有活跃 job 时禁止 `ssh gpu3`，无法手动 reset 或换卡；
   - 每 job 限 1 张 GPU（`--gres=gpu:2` 被拒），无法预约两张再 `CUDA_VISIBLE_DEVICES=1` 钉到健康卡。

**结论 / 当前决策**：H100 路线被坏卡 + 调度无健康感知 + 单卡限制堵死，且未走 IT 重置（drain）GPU 0 的流程。因此本 OCR 任务已**放弃 H100**，全部改用 A 卡——A100 分区（gpu2）跑 `list_a` / `list_b`，A800 分区（gpu1）跑 `list_c`，H100 节点暂停使用，待 IT 修复 GPU 0 后再评估。
