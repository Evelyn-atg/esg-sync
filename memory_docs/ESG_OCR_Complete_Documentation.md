...
- **批处理是关键优化**：单张处理 GPU 利用率仅 40–60%；batch=16 后 H100 约 94.5%、A800 约 47–53%，整体提速 1.5–2×。Token 吞吐恒为 ~162 tok/s（模型算力不变，优化只是减少空闲）。
- **H100 当前不可用**：gpu3 节点 GPU 0（Bus `00000000:23:00.0`）反复崩溃且需硬件重置，Slurm 无健康感知持续把任务分到坏卡，每 job 又限 1 张 GPU 无法绕开。已全面改用 A100 / A800（详见 §4）。
...
3. **无法自助隔离**：`pam_slurm_adopt` 限制无活跃 job 时禁止 `ssh gpu3`；每 job 限 1 张 GPU（`--gres=gpu:2` 被拒），无法 `CUDA_VISIBLE_DEVICES=1` 钉到健康卡。
